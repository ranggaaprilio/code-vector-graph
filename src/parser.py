...
def _load_language(grammar_name: str) -> Language:
  ...
  if grammar_name.lower() in ("javascript", "js", "jsx"):
    if tree_js is None:
      raise RuntimeError("tree_sitter_javascript package is not available in this environment")
    return Language(tree_js.language())
  if grammar_name.lower() in ("typescript", "ts"):
    if tree_ts is None:
      raise RuntimeError("tree_sitter_typescript package is not available in this environment")
...
def strip_comments(source_bytes: bytes, grammar_name: str) -> Tuple[str, Dict[int, int]]:
...
def extract_function_name(tree, node_start_byte: int, node_end_byte: int, source_bytes: bytes) -> Optional[str]:
...
def extract_ast_metadata(tree, source_bytes: bytes) -> dict:
  ...
  def _walk(n):
    nonlocal imports, exports, call_sites, symbols_defined, class_name, is_exported, decorators, parent_function
