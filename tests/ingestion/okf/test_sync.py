...
from code_vector_graph.ingestion.okf.skeleton import build_skeleton, concept_name
...
from code_vector_graph.ingestion.okf.sync import (
    build_prose,
    build_wiki_chunks,
    build_wiki_graph,
    parse_bundle,
...
def test_build_wiki_chunks(tmp_path):
    concepts = parse_bundle(str(_make_bundle(tmp_path)))
    with patch("code_vector_graph.parsing.chunker.chunk_text", side_effect=_fake_chunk_text):
