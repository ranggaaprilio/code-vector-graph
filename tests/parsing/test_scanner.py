...
from code_vector_graph.parsing.scanner import discover_files, is_binary, compute_file_hash, SKIP_DIRS


from tests.conftest import FIXTURES_DIR, PROJECT_ROOT
...
class TestDiscoverFiles:
    ...
    def test_skips_unsupported_extensions(self):
        py_file = PROJECT_ROOT / "src" / "code_vector_graph" / "config.py"
        if py_file.exists():
            results = discover_files(str(PROJECT_ROOT))
            paths = [r["path"] for r in results]
